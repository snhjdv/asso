...
for(fre in 1: length(frq))
{
	...
	paravalues<-c(func.frq,nS,nN,ntrait,paste(traits,collapse=":"),lb.beta,ub.beta,miu,cmn.frq,phenocor[1,2]/nS)
	summary<-c(paravalues,resmat2)
	write(summary,file=paste('out/',"smp",nN,"BBB_v13s.csv", sep=''), append=TRUE,sep=",",ncolumns=length(summary));
