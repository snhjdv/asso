...
trait.simu <- function(geno,lb,ub,miu,
                     cov=NULL, dist=c("normal","cauchy","poisson","binary","T","laplace"),control=NULL)
{
  dist <- match.arg(dist);
  
  #beta<-rnorm(ncol(geno),mean,sd)*abs(log10(maf));
  #set.seed(randseed);
  beta<-runif(ncol(geno),lb,ub);
  alpha<-c(0,1);
  ...
  if(dist=="normal"){
    y<-miu+y.tmp+rnorm(nrow(geno),0,1)+control
  }else if(dist=="cauchy"){
    y<-miu+y.tmp+control;
    y<-rcauchy(length(y),y,.5)
  }else if(dist=="laplace"){
  	y<-miu+y.tmp+control;
  	y<-rlaplace(length(y),y,1)    
  }else if(dist=="poisson"){
    y<-abs(-miu-y.tmp-control)###exp(-miu-y.tmp);
    #y<-#rzipois(length(y), y, pstr0 =0.6) #zero inflated Poisson
    y<-rpois(length(y), y)
  }else if(dist=="binary"){
    y<-1/(1+exp(-miu-y.tmp-control));
