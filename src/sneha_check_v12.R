...
for(fre in 1: length(frq))
{
	...
	lb.beta=0; ub.beta=1; miu=0;
	cmn.frq<-0.05;   #MAF freq
	func.frq<-frq[fre];
...
# 		control4<-rnorm(nN,0,1)
	...
	paravalues<-c(func.frq,nS,nN,ntrait,paste(traits,collapse=":"),lb,ub,miu,cmn.frq,phenocor[1,2]/nS)
	summary<-c(paravalues,resmat2)
	write(summary,file=paste('out/',"smp",nN,"BBB_v12.csv"),append=TRUE,sep=",",ncolumns=length(summary));
